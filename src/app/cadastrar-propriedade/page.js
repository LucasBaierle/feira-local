...
export default function CreatePropertyForm() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const ownerImageUrl = await uploadImage(ownerFile);
      ...
      if (!res.ok) throw new Error(data.error || "Erro ao cadastrar propriedade");
    ...
    <main className="min-h-screen bg-gray-50 p-4 pb-12">
      <div className="max-w-lg mx-auto space-y-6">
        <header className="flex flex-col gap-4 pt-2">
          ...
          <div>
            <h1 className="text-3xl font-black text-gray-900">Cadastrar Propriedade</h1>
            <p className="text-gray-600 text-base mt-1 font-medium">
              Informe os dados da sua barraca para começar a vender.
            </p>
          </div>
        </header>
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-6">
          <section className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex items-center gap-5">
            <div className={`w-20 h-20 rounded-2xl bg-gray-50 flex items-center justify-center overflow-hidden shrink-0 border-2 ${hasSubmitted && !ownerFile ? 'border-red-400' : 'border-green-100'}`}>
              {formData.ownerImageUrl ? (
                ...
                <Store size={32} strokeWidth={2} className={hasSubmitted && !ownerFile ? "text-red-400" : "text-orange-500"} />
              )}
            </div>
          ...
          <section className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 space-y-4">
            ...
            <div>
              ...
              <textarea
                placeholder="Conte um pouco sobre os seus produtos e sua produção..."
          ...
          <section className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <label className={getImageLabelClass(bannerFile)}>
                <Upload size={20} strokeWidth={2.5} className={hasSubmitted && !bannerFile ? "text-red-600" : "text-green-600"} />
            ...
            <div className="space-y-3 pt-2">
              ...
              {formData.ownerImageUrl ? (
                ...
                <div className={`w-24 h-24 rounded-2xl border-2 bg-gray-50 flex items-center justify-center shadow-sm ${hasSubmitted && !ownerFile ? 'border-red-400' : 'border-gray-100'}`}>
                  <Store size={32} strokeWidth={2} className={hasSubmitted && !ownerFile ? "text-red-400" : "text-orange-500"} />
                </div>
              )}
              <label className={getImageLabelClass(ownerFile)}>
                <Upload size={20} strokeWidth={2.5} className={hasSubmitted && !ownerFile ? "text-red-600" : "text-green-600"} />
